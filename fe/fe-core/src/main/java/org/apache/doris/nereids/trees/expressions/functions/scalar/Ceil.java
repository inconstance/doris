...
public class Ceil extends ScalarFunction
    ...
    private Ceil(ScalarFunctionParams functionParams) {
    ...
    public Ceil withChildren(List<Expression> children) {
        ...
        return new Ceil(getFunctionParams(children));
    }
    ...
    public FunctionSignature computePrecision(FunctionSignature signature) {
        if (arity() == 1 && signature.getArgType(0) instanceof DecimalV3Type) {
            ...
            if (mysqlDecimalResultFitsBigInt(resultPrecision)) {
    ...
    private static boolean mysqlDecimalResultFitsBigInt(int resultPrecision) {
        // MySQL changes DECIMAL(q, 0) to BIGINT when its max display length is below 20.
        // A signed scale-zero decimal needs q digit characters plus one sign character.
        int maxDisplayLength = resultPrecision + 1;
        return maxDisplayLength < MYSQL_SIGNED_BIGINT_MAX_DISPLAY_LENGTH;
    }
